...
const VideoGallery = () => {
    ...
    */
    ...
    return (
        ...
        <GenrePanel handleSorting={handleSort} videoLists={videoList} genreFilterHandler={genreFilterHandler}/>
            <Container>
            {
                loading ?
                    ...
                    filteredList && filteredList.length ? filteredList.map(item => {
                        return (
                            <Grid item xs={6} md={3} key={item._id}>
                                    <VideoDisplayCard videos={item} videoList={videoList} fromVideoDetails={true}/>
                            </Grid>
                        )
                    })
                    : <></>
